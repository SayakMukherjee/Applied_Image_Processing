...
/// <returns></returns>
ImageFloat loadDisparity(std::filesystem::path depth_filename)
{
    auto disparity = ImageFloat(depth_filename);

    // Determine scaling factor based on the parent folder name.
    float factor = 1.0f;
    auto parent_dirname = depth_filename.parent_path().stem();
    if (parent_dirname == "half") {
        factor = 2.0f;
    } else if (parent_dirname == "third") {
        factor = 3.0f;
    }
    ...
    for (auto i = 0; i < disparity.data.size(); i++) {
        ...
        if (disparity.data[i] == 0.0f) {
            ...
            disparity.data[i] = (disparity.data[i] * 255.0f + disp_offset) / factor;
        }
    }
...
ImageRGB plotGridMesh(const Mesh& mesh, const glm::ivec2& resolution, const float scale_factor = 1.0f)
{
    ...
    auto scaler = glm::vec2(resolution) * scale_factor;
